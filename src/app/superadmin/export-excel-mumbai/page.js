...
const DataTable = () => {
  ...
  const fetchCustomerData = async () => {
    try {
      ...
      const filteredData = response.data.filter(customer => 
        customer.zone === 'Mumbai' && customer.status === 0
      );
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Mumbai');
      ...
      link.href = url;
      link.download = `Mumbai_${new Date().toISOString().split('T')[0]}.xlsx`;
      link.click();
        ...
        <div style={styles.header}>
          <div style={styles.headerContent}>
            <div style={styles.headerTitle}>
              <h1 style={styles.h1}>Pending Records - Mumbai</h1>
            </div>
